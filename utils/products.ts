export function getProducts() {
  return [
    {
      id: 1,
      name: 'Stackable Chair',
      price: 87.00,
      discountedPrice: 69.00,
      imageUrl: '/chair.png',
    },
    {
      id: 2,
      name: 'Lamp Tool',
      price: 35.00,
      discountedPrice: 29.00,
      imageUrl: '/chair.png',
    },
    {
      id: 3,
      name: 'Arla Accent Chair',
      price: 55.00,
      discountedPrice: 45.00,
      imageUrl: '/chair.png',
    },
    {
      id: 4,
      name: 'Wooden Table',
      price: 120.00,
      discountedPrice: 99.00,
      imageUrl: '/chair.png',
    },
    {
      id: 5,
      name: 'Wooden Tray',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
    },
    {
      id: 6,
      name: 'Magazine Rack',
      price: 75.00,
      discountedPrice: 59.00,
      imageUrl: '/chair.png',
    },
  ];
}

export function getKitchenUtilityProducts() {
  return [
    {
      id: 1,
      name: 'Tissue and Cutlery Stand Global Print',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'MDF and Vinyal Print',
      assembly: 'Carpenter Assembled',
      color: 'Duco Brown',
    },
    {
      id: 2,
      name: 'Tissue and Cutlery Stand Elephant Print',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'MDF and Vinyal Print',
      assembly: 'Carpenter Assembled',
      color: 'Duco Brown',
    },
    {
      id: 3,
      name: 'Cutlery Tray Elephant Print',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'MDF and Vinyal Print',
      assembly: 'Carpenter Assembled',
      color: 'Duco Brown',
    },
    {
      id: 4,
      name: '190 Spoon Stand Gramaphone',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'MDF and Vinyal Print',
      assembly: 'Carpenter Assembled',
      color: 'Duco Brown',
    },
    {
      id: 5,
      name: '190 Spoon Stand Wooden',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Teak Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 6,
      name: '250 Spoon Stand Wooden',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Teak Wood',
      assembly: 'Carpenter Assembled',
      color: 'Brown',
    },
    {
      id: 7,
      name: 'DSK in Gramaphone',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'MDF and Vinyal Print',
      assembly: 'Carpenter Assembled',
      color: 'Duco Brown',
    },
    {
      id: 8,
      name: 'C-8 Coaster Stand Wooden',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Teak Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 9,
      name: 'C-9 Coaster Stand Wooden',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Teak Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 10,
      name: 'C-2 Coaster Stand Wooden',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Teak Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 11,
      name: 'Woden Hotplate Fish Shape',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Rubber Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 12,
      name: 'Woden Hotplate Fish Leaf',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Rubber Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
    {
      id: 13,
      name: 'Woden Hotplate Square',
      price: 25.00,
      discountedPrice: 19.00,
      imageUrl: '/chair.png',
      category: 'Kitchen Utility',
      brand: 'ClasiCraft',
      material: 'Rubber Wood',
      assembly: 'Carpenter Assembled',
      color: 'Natural',
    },
  ];
}

